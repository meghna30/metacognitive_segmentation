...
class GaussianBlurforOOD(object):
    """
    ...
    """
    def __init__(self, kernel_size, std, ID_classes):
        self.kernel_size = kernel_size 
        self.std = std 
        self.ID_classes = ID_classes

    def __call__(self, img, lbl):
        blurred_img = np.array(F.gaussian_blur(img, self.kernel_size, self.std))
        img_np = np.array(img)
        mask_ID = np.isin(lbl, self.ID_classes)
        idx_ID = mask_ID.nonzero()
        idx_OOD = (~mask_ID).nonzero()
        blurred_img[idx_ID[0], idx_ID[1]] = img_np[idx_ID[0], idx_ID[1]]
        blurred_img = Image.fromarray(blurred_img)
        lbl = np.array(lbl)
        lbl[idx_OOD[0], idx_OOD[1]] = 0 
        lbl = Image.fromarray(lbl)
        return blurred_img, lbl
